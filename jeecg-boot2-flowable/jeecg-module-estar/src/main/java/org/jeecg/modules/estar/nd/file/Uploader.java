...
public abstract class Uploader {
        ...
        try {
            ...
            while (iter.hasNext()) {
                ...
                for (MultipartFile multipartFile : multipartFileList) {
                    EstarMultipartFile estarMultipartFile = new EstarMultipartFile(multipartFile);
                    UploadFileResult uploadFileResult = doUploadFlow(estarMultipartFile, uploadFile);
    ...
    protected UploadFileResult doUploadFlow(EstarMultipartFile estarMultipartFile, UploadFile uploadFile) {
        ...
        try {
            rectifier(estarMultipartFile, uploadFile);
            uploadFileResult = organizationalResults(estarMultipartFile, uploadFile);
        } catch (Exception e) {
            throw new UploadException(e);
        }
    ...
    protected abstract void doUploadFileChunk(EstarMultipartFile estarMultipartFile, UploadFile uploadFile)  throws IOException;
    protected abstract UploadFileResult organizationalResults(EstarMultipartFile estarMultipartFile, UploadFile uploadFile);

    private void rectifier(EstarMultipartFile estarMultipartFile, UploadFile uploadFile) {
        String key = "EstarUploader:Identifier:" + uploadFile.getIdentifier() + ":lock";
        String current_upload_chunk_number = "EstarUploader:Identifier:" + uploadFile.getIdentifier() + ":current_upload_chunk_number";

        redisLock.lock(key);
        try {

            if (redisUtil.get(current_upload_chunk_number) == null) {
                redisUtil.set(current_upload_chunk_number, "1", 1000 * 60 * 60);
            }
            int currentUploadChunkNumber = Integer.parseInt((String) redisUtil.get(current_upload_chunk_number));
            ...
            if (uploadFile.getChunkNumber() != currentUploadChunkNumber) {
                ...
                while (redisLock.tryLock(key, 300, TimeUnit.SECONDS)) {
                    ...
                    currentUploadChunkNumber = Integer.parseInt((String) redisUtil.get(current_upload_chunk_number));
            ...
            log.info("文件名{},正在上传第{}块, 共{}块>>>>>>>>>>", estarMultipartFile.getMultipartFile().getOriginalFilename(),uploadFile.getChunkNumber(), uploadFile.getTotalChunks());
            if (uploadFile.getChunkNumber() == currentUploadChunkNumber) {
                doUploadFileChunk(estarMultipartFile, uploadFile);
                log.info("文件名{},第{}块上传成功", estarMultipartFile.getMultipartFile().getOriginalFilename(), uploadFile.getChunkNumber());
                this.redisUtil.getExpire("EstarUploader:Identifier:" + uploadFile.getIdentifier() + ":current_upload_chunk_number");
            }
        } catch (Exception e) {
            log.error("第{}块上传失败，自动重试", uploadFile.getChunkNumber());
            redisUtil.set("EstarUploader:Identifier:" + uploadFile.getIdentifier() + ":current_upload_chunk_number", String.valueOf(uploadFile.getChunkNumber()), 1000 * 60 * 60);
