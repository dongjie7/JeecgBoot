...
//            ossClient.setBucketAcl(newBucket, CannedAccessControlList.PublicRead);
    ...
    */
    ...
    public static byte[] downloadFile(String fileObjectName) throws Exception {
        ...
        try {
            OSSClient ossClient = initOss(endPoint, accessKeyId, accessKeySecret);
            // ossObject包含文件所在的存储空间名称、文件名称、文件元信息以及一个输入流。
            OSSObject ossObject = ossClient.getObject(bucketName, fileObjectName);
            inputStream = ossObject.getObjectContent();
            if (inputStream == null) {
                log.info("file not exist in AmazonS3 store, objectName="+ fileObjectName);
            }
            fileBytes = IOUtils.toByteArray(inputStream);
        } catch (Exception e) {
            log.info("read file from AmazonS3 store error, objectName="+ fileObjectName);
        } finally {
            if (inputStream != null) {
                try {
                    inputStream.close();
