...
public class CommonUtils {
    ...
    /**
     * 根据fileUUid下载文件
     * add by nbacheng for bigscreen
     * @param fileObjectName
     * @return
     */
    public static byte[] downloadFileLocal(String localPath,String fileObjectName) throws Exception {

        byte[] fileBytes = null;
        java.io.File objectFile = null;
        try {
            // 本地文件保存路径
            String filePath = localPath + fileObjectName;
            objectFile = new java.io.File(filePath);
            fileBytes = FileUtils.readFileToByteArray(objectFile);
        } catch (Exception e) {
            log.info("read file from local store error, objectName="+ fileObjectName);
        } finally {
            objectFile = null;
        }
        return fileBytes;
     ...
     * add by nbacheng for bigscreen
     */
    public  static byte[] downloadFile(String fileObjectName,String uploadType) throws Exception {
        byte[] fileBytes;
        if(CommonConstant.UPLOAD_TYPE_MINIO.equals(uploadType)){
            fileBytes = MinioUtil.downloadFile(fileObjectName);
        }else{
            fileBytes = OssBootUtil.downloadFile(fileObjectName);
        }
        return fileBytes;
    }
