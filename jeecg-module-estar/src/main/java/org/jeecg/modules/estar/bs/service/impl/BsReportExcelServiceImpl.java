...
import org.jeecg.modules.estar.bs.mapper.BsReportExcelMapper;
...
import org.jeecg.modules.estar.bs.util.CellType;
 ...
 */
@Service
public class BsReportExcelServiceImpl extends ServiceImpl<BsReportExcelMapper, BsReportExcel> implements IBsReportExcelService {

	@Autowired
    private BsReportExcelMapper reportExcelMapper;
	@Autowired
    private IBsDataSetService dataSetService;

    @Autowired
    private IBsFileService fileService;
	...
	public ReportExcelDto detailByReportCode(String reportCode) {
		QueryWrapper<BsReportExcel> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("report_code", reportCode);
        BsReportExcel reportExcel = reportExcelMapper.selectOne(queryWrapper);
        if (reportExcel != null) {
	...
	public ReportExcelDto preview(ReportExcelDto reportExcelDto) {
		// 根据id查询 报表详情
		QueryWrapper<BsReportExcel> queryExcelWrapper = new QueryWrapper<BsReportExcel>();
		queryExcelWrapper.eq("report_code", reportExcelDto.getReportCode());
		BsReportExcel reportExcel = this.getOne(queryExcelWrapper);
        QueryWrapper<BsReport> queryWrapper = new QueryWrapper<>();
        queryWrapper.eq("report_code", reportExcelDto.getReportCode());
        BsReport report = reportMapper.selectOne(queryWrapper);
        EstarAssert.notNull(reportExcel, ResponseCode.RULE_CONTENT_NOT_EXIST, "reportExcel");
        String setParam = reportExcelDto.getSetParam();

        EstarBeanUtils.copyAndFormatter(reportExcel, reportExcelDto);
...
//        reportExcelDto.setTotal(jsonObject.getJSONObject("rows").size());
	...
	@Override
	public String exportExcel(ReportExcelDto reportExcelDto) {
	...
	@Override
	public String insertShare(ReportShareDto dto) {
		// TODO Auto-generated method stub
		return null;
	}
