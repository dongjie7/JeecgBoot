...
import org.jeecg.modules.estar.bs.config.DruidProperties;
            ...
            try {
                synchronized (lock) {
                    if (!map.containsKey(dataSource.getId())) {
                        ...
                        map.put(Long.valueOf(dataSource.getId()), pool);
                        log.info("创建连接池成功：{}", dataSource.getJdbcUrl());
                    }
                }
                return map.get(Long.valueOf(dataSource.getId()));
            } finally {
            }
        }
    }
