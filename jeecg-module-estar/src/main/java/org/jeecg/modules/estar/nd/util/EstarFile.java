 ...
 */
public class EstarFile {

    private final String path;
    public static final String separator = "/";
    private boolean isDirectory;

    public EstarFile(String pathname, boolean isDirectory) {
        if (StringUtils.isEmpty(pathname)) {
            throw new RuntimeException("file name format error，pathname:" + pathname);
        }
        this.path = formatPath(pathname);
        this.isDirectory = isDirectory;
    }

    public EstarFile(String parent, String child, boolean isDirectory) {
        if (StringUtils.isEmpty(child)) {
            throw new RuntimeException("file name format error，parent:" + parent +", child:" + child);
        }
        if (parent != null) {
    ...
    public EstarFile getParentFile() {
        String parentPath = this.getParent();
        return new EstarFile(parentPath, true);
    }
