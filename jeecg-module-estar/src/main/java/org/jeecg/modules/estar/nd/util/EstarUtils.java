...
public class EstarUtils {
    ...
    public static File getLocalSaveFile(String fileUrl) {
        String localSavePath = EstarUtils.getStaticPath() + fileUrl;
        return new File(localSavePath);
    }

    public static File getCacheFile(String fileUrl) {
        String cachePath = EstarUtils.getStaticPath() + "cache" + File.separator + fileUrl;

        return new File(cachePath);
    }

    public static File getTempFile(String fileUrl) {
        String tempPath = EstarUtils.getStaticPath() + "temp" + File.separator + fileUrl;
        File tempFile = new File(tempPath);
        File parentFile = tempFile.getParentFile();
        if (!parentFile.exists()) {
            boolean result = parentFile.mkdirs();
            if (!result) {
                throw new RuntimeException("创建temp目录失败：目录路径："+ parentFile.getPath());
            }
        }
    ...
    public static File getProcessFile(String fileUrl) {
        String processPath = EstarUtils.getStaticPath() + "temp" + File.separator + "process" + File.separator + fileUrl;
        File processFile = new File(processPath);
        File parentFile = processFile.getParentFile();
        if (!parentFile.exists()) {
            boolean result = parentFile.mkdirs();
            if (!result) {
                throw new RuntimeException("创建process目录失败：目录路径："+ parentFile.getPath());
            }
        }
        return processFile;
    }
    ...
    public static String getUploadFileUrl(String identifier, String extendName) {
        ...
        File dir = new File(EstarUtils.getStaticPath() + path);
    ...
    public static String formatPath(String path) {
        path = EstarUtils.pathSplitFormat(path);
        if ("/".equals(path)) {
            return path;
        }
        if (path.endsWith("/")) {
